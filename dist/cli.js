...
function argumentsFrom(values) {
    const options = { target: ".", diff: false, json: false, badge: false, explain: false, baseline: false };
    for (let index = 0; index < values.length; index += 1) {
        ...
        else if (value === "--fail-over")
            options.failOver = numeric(values[++index], value);
        else if (value === "--top")
            options.top = numeric(values[++index], value);
...
async function main() {
    ...
    if (options.badge)
        ...
        process.stdout.write(renderTty(result, { serious, explain: options.explain, top }));
    else
        process.stdout.write(renderPlain(result, { serious, explain: options.explain, top }));
    const threshold = options.failOver ?? config["fail-over"];
    if (threshold !== undefined && result.score >= threshold)
        process.exitCode = 1;
}
main().catch((error) => {
    process.stderr.write(`slop-check: ${error.message}\n`);
    process.exitCode = 2;
});
