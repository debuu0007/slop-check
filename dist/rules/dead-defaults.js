...
export const rule = {
    ...
    check(path, content, diff) {
        ...
        for (const match of content.matchAll(declaration)) {
            ...
            if (/^[ \t]+/.test(declarationLine))
                continue;
            // Public API: the callers are outside this repository by construction.
            if (python ? !match[1].startsWith("_") : /\bexport\b/.test(declarationLine))
                continue;
            ...
            if (calls.length < MINIMUM_CALL_SITES)
                continue;
...
//# sourceMappingURL=dead-defaults.js.map