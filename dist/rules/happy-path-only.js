...
export const rule = {
    ...
    check(path, content, diff) {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            if (!ioPattern.test(lines[index]) && !(/\.py$/i.test(path) && pythonOpenPattern.test(lines[index])))
                continue;
            if (declarationPattern.test(lines[index]) || contextManagerPattern.test(lines[index]))
                continue;
            const nearby = lines.slice(Math.max(0, index - 12), index + 13).join("\n");
            if (/\b(?:try|catch|except|\.catch\s*\(|on\s*\(\s*['"]error|if\s+[^\n]*(?:error|status|ok))\b/i.test(nearby))
                continue;
            const line = index + 1;
...
//# sourceMappingURL=happy-path-only.js.map