...
export const roasts = ["This helper has been independently rediscovered.", "The repository already knew this one.", "Reuse was considered and then reimplemented.", "A familiar solution wearing a new function name."];
 ...
 */
function asyncTwins(left, right) {
    const bare = (name) => name.replace(/a?sync/gi, "").replace(/_+/g, "_").replace(/^_|_$/g, "").toLowerCase();
...
function candidatePositions(block, position, owners) {
    let smallest = [];
    for (const shingle of block.shingles) {
        const candidates = owners.get(shingle) ?? [];
        if (candidates.some((candidate) => candidate < position) && (!smallest.length || candidates.length < smallest.length))
            smallest = candidates;
    }
...
export const rule = {
    ...
    check(path, content, diff) {
        ...
        for (const block of index.functionsByPath.get(path) ?? []) {
            ...
            if (duplicate && isIncluded(block.line, diff))
                findings.push(makeFinding(this, path, block.line, lineText(content, block.line)));
        }
        return findings;
    },
};
