...
function groupText(group, serious, color) {
    ...
    if (remaining)
        lines.push(`   │ ${color ? ansi.dim : ""}… and ${remaining} more in this file${color ? ansi.reset : ""}`);
    if (!serious)
        lines.push(`   │ ${color ? ansi.dim : ""}${shown[0]?.roast ?? ""}${color ? ansi.reset : ""}`);
    return lines.join("\n");
}
...
export function renderPlain(result, options = {}) {
    ...
    if (options.explain)
        lines.push(explanation(result));
    const selected = result.groups.slice(0, options.top ?? result.groups.length);
    if (selected.length)
        lines.push("", ...selected.map((group) => groupText(group, Boolean(options.serious), false)).flatMap((item, index) => index ? ["", item] : [item]));
    if (result.grade === "A")
        lines.push("", "Grade A? Prove it: add the badge with `slop-check --badge > slop-check.svg`.");
    return `${lines.join("\n")}\n`;
}
export function renderTty(result, options = {}) {
    const width = 58;
    const inner = width - 4;
    ...
    if (options.explain)
        report.push("", `  ${explanation(result)}`);
    const selected = result.groups.slice(0, options.top ?? result.groups.length);
    if (selected.length)
        report.push("", ...selected.map((group) => `  ${groupText(group, Boolean(options.serious), true).replace(/\n/g, "\n  ")}`).flatMap((item, index) => index ? ["", item] : [item]));
    if (result.grade === "A")
        report.push("", "  Grade A? Prove it: add the badge with `slop-check --badge > slop-check.svg`.");
    return `${report.join("\n")}\n`;
}
function xml(value) {
