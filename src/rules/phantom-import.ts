...
function pythonTarget(specifier: string, fromPath: string): string {
...
export const rule: Rule = {
  ...
  check(path, content, diff): Finding[] {
     ...
     */
    if (!present?.size || diff?.repositoryComplete !== true) return [];
    const findings: Finding[] = [];
    const python = /\.py$/i.test(path);

    const resolves = (target: string): boolean => {
      if (!target) return true;
      if (python) return present.has(`${target}.py`) || present.has(`${target}/__init__.py`);
    ...
    for (const match of content.matchAll(python ? pythonImport : jsImport)) {
      ...
      if (!python && NON_SOURCE_ASSET.test(specifier) && !SOURCE_EXTENSION.test(specifier)) continue;
      ...
      if ([...present].some((file) => file.startsWith(`${target}/`) || file.startsWith(`${target}.`))) continue;
