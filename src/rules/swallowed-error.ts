 ...
 */
function enclosingDefinition(lines: readonly string[], line: number): string {
  ...
  for (let index = line - 2; index >= 0; index -= 1) {
    ...
    if (/^\s*(?:class|def)\b/.test(text)) return "";
  }
  return "";
}
...
export const rule: Rule = {
  ...
  check(path, content, diff) {
    ...
    for (const match of maskComments(content, path).matchAll(expression)) {
      ...
      if (enclosingDefinition(lines, line) === "__del__") continue;
