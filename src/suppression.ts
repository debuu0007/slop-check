...
/** An empty set means a bare directive, which covers every rule in its scope. */
type RuleSet = ReadonlySet<string>;

interface FileDirectives {
  file: RuleSet[];
  /** Keyed by the line number the directive applies to, not the line it sits on. */
  byLine: Map<number, RuleSet[]>;
}

function namedRules(raw: string | undefined): RuleSet {
  const named = (raw ?? "").replace(/\*\/.*$/, "").trim();
  return new Set(named ? named.split(/[\s,]+/).filter(Boolean) : []);
}

function directivesFor(content: string): FileDirectives {
  const directives: FileDirectives = { file: [], byLine: new Map() };
  ...
  for (const [index, text] of lines.entries()) {
    ...
    for (const match of text.matchAll(DIRECTIVE)) {
      ...
      if (scope === "file") { directives.file.push(rules); continue; }
...
function covers(sets: readonly RuleSet[] | undefined, ruleId: string): boolean {
...
export function applySuppressions(findings: readonly Finding[], contents: ReadonlyMap<string, string>): SuppressionResult {
  const cache = new Map<string, FileDirectives>();
  const kept: Finding[] = [];
  let suppressed = 0;
  for (const finding of findings) {
    const content = contents.get(finding.path);
    if (content === undefined) { kept.push(finding); continue; }
    let directives = cache.get(finding.path);
    if (!directives) { directives = directivesFor(content); cache.set(finding.path, directives); }
