...
export async function collectFiles(target: string, ignorePatterns: readonly string[] = []): Promise<{ files: SourceFile[]; skippedFiles: number; knownPaths: string[] }> {
  ...
  async function visit(path: string): Promise<void> {
    ...
    if (!info.isDirectory() && isSupported(displayPath)) knownPaths.push(displayPath);
    if (ignores.some((pattern) => pattern.test(displayPath))) return;
    if (info.isDirectory()) {
      if (path !== absolute && (ignoredDirectories.has(basename(path)) || basename(path).startsWith("."))) return;
      const entries = await readdir(path);
      await Promise.all(entries.sort().map((entry) => visit(resolve(path, entry))));
      return;
    }
    if (!isSupported(displayPath)) return;
    if (/\.min\.[^.]+$/i.test(displayPath)) { skippedFiles += 1; return; }
    if (isGeneratedFile(displayPath)) return;
    if (isTestPath(displayPath)) return;
    if (isIllustrativePath(displayPath)) return;
    if (info.size > 1024 * 1024) { skippedFiles += 1; return; }
    let content: string;
    try {
    ...
    if (lines.some((line) => line.length > 1000)) { skippedFiles += 1; return; }
