import type { DiffContext, IndexedCall, IndexedFunction, RepoIndex, SourceFile } from "./types.js";
...
export function isReservedLiteral(value: string): boolean {
...
function tokens(body: string): string[] {
  return body.toLowerCase()
    .replace(/(['"])((?:\\.|(?!\1).)*)\1/g, (_match, _quote, value: string) => ` string_${hash(value)} `)
    .replace(/\b\d+(?:\.\d+)?\b/g, (value) => ` number_${value.replace(".", "_")} `)
    .match(/[a-z_$][\w$]*|===|!==|=>|\S/g) ?? [];
}

function functionMetadata(path: string, name: string, body: string, index: number, content: string): IndexedFunction {
  const bodyTokens = tokens(body);
  const shingles = new Set<string>();
  for (let offset = 0; offset <= bodyTokens.length - 4; offset += 1) shingles.add(bodyTokens.slice(offset, offset + 4).join(" "));
...
export function splitTopLevel(list: string): string[] {
  ...
  for (let index = 0; index < list.length; index += 1) {
    ...
    if (quote) {
      current += character;
      if (character === "\\") current += list[index += 1] ?? "";
...
function topLevelArity(argumentList: string): number {
 ...
 */
function keywordNames(argumentList: string): Set<string> {
...
function extractFunctions(file: SourceFile): IndexedFunction[] {
  const blocks: IndexedFunction[] = [];
  if (/\.py$/i.test(file.path)) {
    const expression = /^(?:async\s+)?def\s+(\w+)\s*\([^)]*\)\s*(?:->[^:]+)?\s*:\s*\n((?:[ \t]+[^\n]*(?:\n|$)){3,})/gm;
    for (const match of file.content.matchAll(expression)) blocks.push(functionMetadata(file.path, match[1], match[2], match.index ?? 0, file.content));
  } else {
    ...
    for (const match of file.content.matchAll(expression)) {
      ...
      if (body.split("\n").filter((line) => line.trim()).length >= 5) blocks.push(functionMetadata(file.path, match[1] ?? match[2], body, index, file.content));
    }
  }
  return blocks;
}

export function buildRepoIndex(files: readonly SourceFile[], contexts?: ReadonlyMap<string, DiffContext>): RepoIndex {
  const functions = files.flatMap(extractFunctions).sort((a, b) => `${a.path}:${a.index}`.localeCompare(`${b.path}:${b.index}`));
  const functionsByPath = new Map<string, IndexedFunction[]>();
  const shingleOwners = new Map<string, number[]>();
  functions.forEach((block, position) => {
    block.position = position;
    const own = functionsByPath.get(block.path) ?? []; own.push(block); functionsByPath.set(block.path, own);
    for (const shingle of block.shingles) { const owners = shingleOwners.get(shingle) ?? []; owners.push(position); shingleOwners.set(shingle, owners); }
  });
  const literalCounts = new Map<string, number>();
  const callsByName = new Map<string, IndexedCall[]>();
  ...
  for (const file of files) {
    ...
    for (const match of file.content.matchAll(literalRegex)) {
      ...
      if (isReservedLiteral(value)) continue;
    ...
    for (const match of file.content.matchAll(callRegex)) {
      ...
      const calls = callsByName.get(match[1]) ?? []; calls.push(call); callsByName.set(match[1], calls);
    }
  }
  const addedNormalizedLines = new Set<string>();
  const byPath = new Map(files.map((file) => [file.path, file]));
  for (const [path, context] of contexts ?? []) {
    const file = byPath.get(path);
    if (!file) continue;
    const lines = contentLines(file.content);
    for (const line of context.changedLines ?? []) { const normalized = lines[line - 1]?.trim().replace(/\s+/g, " "); if (normalized) addedNormalizedLines.add(normalized); }
  }
