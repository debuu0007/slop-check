...
import type { AnalysisResult, AnalyzeOptions, DiffContext, FileResult, Finding, FindingGroup, SourceFile } from "./types.js";
...
function score(weightedHits: number, lines: number): number {
  if (!weightedHits || !lines) return 0;
 ...
 */
function dampedHits(hits: number): number {
...
function groupFindings(findings: readonly Finding[]): FindingGroup[] {
  ...
  for (const finding of findings) {
    ...
    if (existing) { existing.findings.push(finding); existing.count += 1; continue; }
...
export function analyzeFiles(input: readonly SourceFile[], options: AnalyzeOptions = {}): AnalysisResult {
  ...
  for (const file of files) {
    ...
    const context: DiffContext = { ...supplied, repository: files, repoIndex, knownPaths, repositoryComplete: options.completeRepository === true };
  ...
  const hitsByRule = new Map<string, Finding[]>();
  for (const finding of allFindings) {
    const list = hitsByRule.get(finding.ruleId);
    if (list) list.push(finding); else hitsByRule.set(finding.ruleId, [finding]);
  }
  ...
  for (const finding of allFindings) ruleHits[finding.ruleId] += 1;
